...
translations = dict({
    "ru": dict({
        ...
        action_messages[Actions.ADD_INTERVAL_REMINDER_SET_REPETITIONS]: "Пожалуйста, выберите количество приемов препарата в день (первый \
прием будет без напоминания).",
        action_messages[Actions.ADD_INTERVAL_REMINDER_SET_INTERVAL]: "Пожалуйста, выберите количество часов между приемами.",
...
https://github.com/stonekite/eshkoleso_bot (./handlers/misc.py, метод add_timezone).",
        action_messages[Actions.ADD_TIMEZONE_SET_TIMEZONE]: "🌍 Отправить локацию",
